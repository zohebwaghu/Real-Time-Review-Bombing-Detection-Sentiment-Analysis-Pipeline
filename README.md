...
Notes:
- AWS credentials are only required for the S3/Athena portions of the pipeline.
- You can still run local unit tests and most Docker services without valid AWS keys.
...
Required files:
- `data/review.json` is required for model training and producer-based pipeline demos.
- `data/business.json` is required for downstream business-level analysis workflows.
- Without these files, the Docker stack can start, but end-to-end data flow and training steps will not complete.
...
- `python3 -m unittest discover -s tests -v` should pass the pure-Python checks without requiring Docker, Spark, or AWS.
- Sentiment-model artifact tests are skipped until `models/sklearn_lr.pkl` and `models/tfidf_vectorizer.pkl` have been generated by `models/train_sentiment_model.py`.
- If Docker startup fails on Spark image resolution, make sure your Compose file uses a valid Apache Spark image tag.
- Full end-to-end verification requires all of the following:
  - Docker services running successfully
  - Yelp dataset files present under `data/`
  - Trained model artifacts under `models/`
  - AWS credentials plus an accessible S3 bucket for Parquet/Athena steps
